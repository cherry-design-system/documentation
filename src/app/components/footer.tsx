...
const StyledFooter = styled.footer<{ theme: Theme }>`
  background: ${({ theme }) =>
    theme.isDark ? rgba(theme.colors.grayLight, 0.5) : rgba(theme.colors.primaryLight, 0.2)};
  margin-top: 60px;
`;
...
const StyledHr = styled.hr<{ theme: Theme }>`
  ...
  width: 100%;
  background: ${({ theme }) =>
    theme.isDark ? theme.colors.grayLight : rgba(theme.colors.primaryLight, 0.5)};
`;
      ...
      <Container $lgPadding={20}>
        ...
        <Grid $xsCols={1} $lgCols={2}>
          <StyledParagraph $align="left">
            ...
            <a href="https://www.riangle.com/?ref=cherry.design" target="_blank">
              Riangle
            </a>
            .<br />
            Created by{" "}
  ...
  return (
    <StyledFooter>
      <Grid $xsCols={1} $lgCols={3}>
        <Col $lgSpan={2}>
          <StyledParagraph>Copyright © {year} Cherry. All rights reserved.</StyledParagraph>
        </Col>
        <StyledLink
