...
Browse the full set at [lucide.dev/icons](https://lucide.dev/icons). In code, import a glyph by its PascalCase name (for example `Settings`, `Trash2`, `ImageUp`) from `lucide-react`, a peer dependency installed alongside Cherry, and render it with the [Icon](/code/icon) component. Only the glyphs you import end up in your bundle. An icon chosen at runtime, from content or a user setting, goes through `DynamicIcon` instead, documented on the same page. Designers can use the official [Lucide Figma plugin](https://www.figma.com/community/plugin/939567362549682242/lucide-icons) to place the same icons in mockups.
