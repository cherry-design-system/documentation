...
Each intent brings its own icon: `CircleAlert` for note, `Info` for info, `TriangleAlert` for warning, `OctagonAlert` for danger, and `Check` for success. Override it with `$icon`, for example `<Callout $type="note" $icon={Lightbulb}>` with `Lightbulb` imported from `lucide-react`.
...
<Field value="$icon" type="IconSource">
  Overrides the intent's default icon, or supplies one for an untyped callout: a Lucide component imported from `lucide-react`, or a rendered element.
</Field>
