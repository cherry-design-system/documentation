...
<Field value="$titleIcon" type="IconSource | null">
  Icon before the title: a Lucide component imported from `lucide-react`, or a rendered element. Defaults to `Sparkles`; pass `null` for none.
</Field>
...
<Callout type="note">
  A reset action pairs naturally with the header: `$actions={<IconButton onClick={reset} aria-label="New chat"><Icon icon={RotateCcw} /></IconButton>}` with `reset` from `useChat()` and `RotateCcw` imported from `lucide-react`.
</Callout>
