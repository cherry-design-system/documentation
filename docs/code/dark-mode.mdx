...
For a custom control, use `toggleTheme` (or `setTheme`) from `ThemeContext` yourself. Persistence is handled by the provider:
...
Theme changes persist to the `theme` cookie and `localStorage` directly. No API route is needed.
