...
Install the Cherry library from npm along with its peer dependencies. Import components directly into your project. Perfect for smaller projects or rapid development. The library requires `react` and `react-dom` (^18 or ^19), `styled-components` (^6) and `lucide-react` (^1.31) as peer dependencies; icon glyphs are imported from `lucide-react` directly.

<CodeTabs tabs={[{ label: "npm", code: "npm install cherry-styled-components react react-dom styled-components lucide-react" }, { label: "pnpm", code: "pnpm add cherry-styled-components react react-dom styled-components lucide-react" }, { label: "yarn", code: "yarn add cherry-styled-components react react-dom styled-components lucide-react" }]} />
