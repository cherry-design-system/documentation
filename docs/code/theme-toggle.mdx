...
Bind the **Cmd/Ctrl+Shift+L** shortcut with the `$shortcut` prop. While the toggle is mounted, pressing it flips the theme without a click:
...
<Field value="$shortcut" type="boolean">
  ...
  flipping the theme without a click. Off by default.
</Field>
