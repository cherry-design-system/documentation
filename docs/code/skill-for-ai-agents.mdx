...
- **The app is wrapped in a Cherry provider**, uses the layout primitives (`Container`, `Grid`, `Col`, `Flex`, `Box`, `MaxWidth`, `Space`), and renders icons with `<Icon icon={Glyph} />`, glyphs imported from `lucide-react`.
