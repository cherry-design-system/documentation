...
- **Web** - Overall web design, including a sample page composed entirely of Cherry components, shown side by side in light and dark mode.
- **Components** - Reusable design elements that streamline your workflow. Every component is a variant set bound to the Figma variables, so all of them adapt to dark mode automatically: Button, IconButton, Input, Password, Textarea, Select, Checkbox, Radio, Toggle, Range, Modal, Toast, Accordion, Tabs, Dropzone, and AvatarDropzone.
...
Overlay and utility components (IconButton, Password, Modal, Toast, Accordion, Tabs, Dropzone, AvatarDropzone) are previewed directly in the Components page and composed together on the Web page's sample screens.
